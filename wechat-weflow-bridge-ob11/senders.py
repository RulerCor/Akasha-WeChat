...
def create_sender() -> BaseSender:
    ...
    if SEND_METHOD == "weflow_api":
        ...
        # 提醒（保留自排查结论）：WeFlow 实测只有读取接口，没有发送接口，
        # /api/v1/message、/api/v1/send、/api/v1/messages/send 全部 404。
        # 这个分支目前必然失败，正常请用 send_method = "uia"。
        log.warning("⚠️ send_method=weflow_api，但 WeFlow 目前没有发送接口，消息会发送失败。"
                    "请改用 send_method=\"uia\"。")
